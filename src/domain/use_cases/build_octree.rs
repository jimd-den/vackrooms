...
impl BuildOctreeUseCase {
    ...
    fn build_recursive(
        ...
        if depth == 0 {
            let (v_type, color, ll) = self.get_voxel_attrs(grid, x, y, z);
            return SvoNode::Leaf {
                voxel_type: v_type,
                color,
                light_level: ll,
        ...
        let mut children = [SvoNode::Leaf { voxel_type: 0, color: 0, light_level: 0 }; 8];
        ...
        for octant_idx in 0..8 {
            ...
            children[octant_idx as usize] = self.build_recursive(
                grid,
                nodes,
                x + dx,
                y + dy,
    ...
    fn get_voxel_attrs(&self, grid: &VoxelGrid, x: u32, y: u32, z: u32) -> (u8, u32, u8) {
        if x >= grid.width() as u32 || y >= grid.height() as u32 || z >= grid.depth() as u32 {
            return (0, 0, 0); // Out of bounds is air
        }
