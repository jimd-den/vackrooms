...
fn cast_ray(
    ...
    let mut t_max_x = if dx != 0.0 { (x.floor() + if dx > 0.0 { 1.0 } else { 0.0 } - x) / dx } else { f32::INFINITY };
    let mut t_max_y = if dy != 0.0 { (y.floor() + if dy > 0.0 { 1.0 } else { 0.0 } - y) / dy } else { f32::INFINITY };
    let mut t_max_z = if dz != 0.0 { (z.floor() + if dz > 0.0 { 1.0 } else { 0.0 } - z) / dz } else { f32::INFINITY };
    ...
    for _ in 0..max_steps {
        ...
        if ix < 0 || iy < 0 || iz < 0 || ix >= grid.width() as isize || iy >= grid.height() as isize || iz >= grid.depth() as isize {
            break;
        }
    ...
    for z in 0..d {
        for y in 0..h {
            for x in 0..w {
                ...
                if x > 0 && grid.get(x - 1, y, z) == VOXEL_AIR { px -= 0.55; }
                else if x < w - 1 && grid.get(x + 1, y, z) == VOXEL_AIR { px += 0.55; }
                else if y > 0 && grid.get(x, y - 1, z) == VOXEL_AIR { py -= 0.55; }
                else if y < h - 1 && grid.get(x, y + 1, z) == VOXEL_AIR { py += 0.55; }
                else if z > 0 && grid.get(x, y, z - 1) == VOXEL_AIR { pz -= 0.55; }
                else if z < d - 1 && grid.get(x, y, z + 1) == VOXEL_AIR { pz += 0.55; }
                else {
                ...
                for _ in 0..rays_per_voxel {
                    ...
                    if let Some((_hx, _hy, _hz, hit_type)) = cast_ray(grid, px, py, pz, dx, dy, dz, max_steps) {
