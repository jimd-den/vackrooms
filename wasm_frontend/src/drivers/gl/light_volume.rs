...
/// Uploads the chunk's light volume placeholder (see module docs): a single
/// black texel, linearly filtered, clamped — sampling anywhere yields zero
/// baked irradiance.
pub fn upload_light_volume(gl: &Gl, _mesh: &SurfaceMeshPayload) -> Result<WebGlTexture, JsValue> {
    let texture = gl
        .create_texture()
